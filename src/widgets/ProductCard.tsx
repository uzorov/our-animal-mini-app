...
export const ProductCard = ({ name, image, price, shop, offers, id }: ProductCardProps) => {
    ...
    const [modalProduct, setModalProduct] = useState<null | ProductCardProps>(null);
    ...
    // Для модального окна используем либо текущий товар, либо выбранный offer
    const modalData = modalProduct || { id, name, image, price, shop, offers };
    // Для offers в модалке: исключаем текущий товар, чтобы не было дублирования
    const modalOffers = [
        { id, name, image, price: Number(price) || 0, shop, category: '' },
        ...offers
    ].filter((o, idx, arr) => o.id !== modalData.id && arr.findIndex(x => x.id === o.id) === idx);
    ...
    return (
        ...
        >
            ...
            {(open || modalProduct) && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50" onClick={() => { setOpen(false); setModalProduct(null); }}>
                    <div className="bg-white rounded-xl p-4 w-[95vw] max-w-md relative" onClick={e => e.stopPropagation()}>
                        <div className="flex flex-row gap-3 mb-2">
                            <img src={modalData.image} alt={modalData.name} style={{ width: 100, height: 100, borderRadius: 10, objectFit: 'cover' }} />
                            <div className="flex-1 flex flex-col items-start">
                                <div className="text-primary font-bold text-lg mb-1">{modalData.name}</div>
                                <div className="text-darkRed font-bold text-xl mb-2">{modalData.price} ₽</div>
                                <div className="flex gap-2">
                                    ...
                                    <button className="bg-darkRed text-white font-semibold rounded-xl px-3 py-1" onClick={() => { addToCart({ ...modalData, quantity: 1 }); setOpen(false); setModalProduct(null); }}>В корзину</button>
                                </div>
                            </div>
                        </div>
                        <div className="mt-2">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2" style={{ borderRadius: 8 }}>
                                {modalOffers.map((offer, idx) => (
                                    <div
                                        key={offer.id || idx}
                                        className="bg-white border border-primary rounded-xl shadow p-2 flex items-center gap-2 cursor-pointer hover:bg-rose-100 transition"
                                        onClick={() => setModalProduct({ ...offer, offers: modalOffers.filter(o => o.id !== offer.id) })}
                                    >
                        ...
                        <button onClick={() => { setOpen(false); setModalProduct(null); }} className="absolute top-2 right-2 text-gray-400 hover:text-gray-700 text-xl">×</button>
                    </div>
                </div>
            )}
        </div>
